...
- **12.1 Mode choice at Setup.** Two paths: *use my class marksheet*
  ...
  > **Amended 2026-09-09 by step 13 (plan.md §18).** The reasoning above is
  > correct and is not withdrawn; what changed is its price. Once a
  > semester holds four sections, "a fresh upload per quiz" costs
  > twenty-four uploads, and the roster turns out to belong to the
  > *section* rather than the quiz. Step 13 moves the upload onto the
  > section and replaces this rule rather than dropping it: staleness
  > becomes **visible** instead of impossible — every workbook export
  > states which copy it is writing into, offers a first-class **Re-pick**,
  > and re-caches the bytes just written. Until step 13's Phase B lands,
  > this rule is still the one in force.
...
## Step 13 — Multi-course, multi-section persistence

**Goal.** Hold a semester instead of a sitting. One faculty member,
several courses, several sections per course — grade any of them, in any
order, without one destroying another, and without re-entering the class
list every quiz.

Specced 2026-09-09 from a design discussion grounded in the user's real
semester: **one section of CSE100, one of CSE200, and two of CSE203**.
Folded into plan.md §18, which carries the fuller rationale — this is that
section's concrete build order, the same relationship step 12 has to plan
§17 and step 11 has to plan §13. Like both of those, it is a deliberate
extension beyond plan §13's MVP scope, and like both it leaves the
existing path intact: the plain download does not change, and the capture
→ review → confirm loop does not change at all.

### What this changes about the working model

Everything before this step assumed one quiz at a time, and said so in
code rather than in prose: `db.ts`'s `config` store holds a single
overwritten value, `records` is one flat store with no quiz field, and
`resetAll()` is the only way between sessions. For four sections that
means eight config entries and eight class-list uploads per round of
quizzes, eight destructive resets, and no way to look at last week.

The replacement is two durable entities — a **Section** (course code,
label, semester, ID digits, the class list, one workbook file) and an
**Assessment** (one quiz's question config plus its records). Course is
derived from `section.courseCode`, not stored. See plan §18 for why two
entities rather than four.

### Before you start

Read plan.md §18 in full, and then these three, because each one is a
decision this step has to either honour or consciously reverse:

- **`Setup.tsx`'s current shape**, in particular that `config === null` in
  `App.tsx` *is* the router. There are no routes today; this step
  introduces the first real screen graph and that is most of its risk.
- **§17's 12.1 rule and the reason given for it** — a fresh workbook
  upload per quiz, so the app never writes into a stale copy. This step
  reverses it. Read plan §18's "staleness" subsection before writing any
  of Phase B; the reversal is only safe because of what replaces it.
- **`db.ts`'s `meta` store and `getSourceId()`**, and CLAUDE.md's rule
  about it. The migration and the purge both touch storage, and neither
  may clear it.

### Rules this step is bound by

Load-bearing, each a decision rather than a detail:

1. **The scan loop does not change.** Capture, recognize, review, confirm,
   save. It runs thirty times a class and is tuned; plan §11's "don't add
   a tap" applies unchanged. Everything here sits before it or after it.
2. **No backend changes.** `/api/scan` takes the config per request and
   stores nothing. Sections and semesters are entirely a frontend concern
   — if any substep here seems to need an endpoint, it is wrong.
3. **The migration folds an in-flight session; it never drops one.**
   Someone may be mid-quiz when the new bundle loads.
4. **Duplicate detection is scoped to the assessment**, and ships with the
   schema, not after it. A shared serial across two courses is normal.
5. **The purge is never automatic**, never on a timer, and is blocked
   outright by any assessment that was never exported.
6. **The plain download stays byte for byte**, as the escape hatch it
   already is for every other flow in this app.
7. **Staleness is made visible, never assumed away.** Every workbook
   export states which copy it is writing into and offers a re-pick.
8. **No new dependency.** Everything here is IndexedDB, React state and
   code that already exists.

### Substeps

**13.0 — Amend the specs first. DONE 2026-09-09**, before any code, since
three documents asserted things this step makes false and leaving them is
exactly the drift the 2026-08-31 audit kept finding:

- plan §17's **12.1 "a fresh upload is required per quiz"** — reversed
  here, with plan §18's staleness rule as the replacement. Amend in place,
  naming the reversal rather than quietly editing the sentence.
- **CLAUDE.md's "Deferred — don't build these"**, which lists
  "server-side database and multi-quiz history" as one item. Split it:
  *local* multi-quiz history is this step; a *server-side* database and
  multi-user auth stay deferred, and for the same reason as ever.
- **README.md's "Deliberately not built"**, same sentence, same split.
  Plus its "one quiz session, one class" framing near the top.

All three are amended, plus two more found while doing it: plan §8 and §11
gained forward-references (the models are *wrapped*, not replaced —
`QuizConfig`'s fields survive inside an `Assessment` and the §9 API
contract is untouched; the Library and Section screens arrive ahead of the
existing four), and two stale "not yet built" claims about step 12 were
corrected in plan §13 and CLAUDE.md's Deferred section.

#### Phase A — the schema and the library — **DONE 2026-09-10**

The whole phase is invisible to a user except that a second quiz stops
destroying the first. Shipped before anything cosmetic, exactly as
specced — plus Phase B in the same pass (see its own note below), because
Phase A alone would have shipped a real regression: no way to attach a
class list to anything until Phase B's code existed too.

- **13.1 DB v5, done.** `sections` and `assessments` stores added,
  `StudentRecord` gained `assessmentId`, `records` gained a
  `by-assessment` index. `config` is genuinely retired — a fresh v5
  install never creates it at all — not repurposed.
- **13.2 The migration, done, tested as three cases.** A v1-v4 database
  with a saved config folds into one Section (`courseCode: "Imported"`)
  and one Assessment holding every existing record, stamped with the new
  `assessmentId`; an empty v1-v4 database produces nothing; a v4 database
  that also had a persisted roster folds it straight into the migrated
  Section's `roster`/`workbook` fields (Phase B's fields exist on `Section`
  from the start for exactly this reason — see 13.9's note). All three
  are `db.test.ts` cases, run against a real `openDB` at the old version
  first, not simulated. One implementation subtlety worth recording: the
  migration reads and deletes `config`/`rosterUpload` from *inside* the
  same versionchange transaction the new stores are created in, chaining
  through the existing v3 cursor-migration's own proven pattern
  (promise-chained requests keep an IndexedDB upgrade transaction alive;
  `db.deleteObjectStore()` is then called synchronously at the end of
  that chain, still inside the active transaction) — this is not
  speculative; fake-indexeddb genuinely exercises it and the tests catch
  a wrong ordering.
- **13.3 `sections.ts`, done.** `groupSections` (semester → course →
  section, natural-numeric label sort so "10" doesn't sort before "2"),
  `isDuplicateSection`, `sectionDisplayLabel`, `assessmentsForSection`,
  and `assessmentConfig(assessment, section)` — the one function that
  combines the two into the `QuizConfig` shape every scan-loop screen
  already takes, reading `idDigits` from the SECTION rather than
  duplicating it onto `Assessment` (plan.md §18 — a copy is how the two
  would drift). 14 tests, pure, no DOM, no IndexedDB.
- **13.4 `Library.tsx`, done.** The new entry point, self-fetching its own
  sections/assessments/per-assessment scanned counts on mount — same
  pattern Setup.tsx's own initial load always used. Also absorbed two
  things that had nowhere else to live once Setup.tsx was retired (see
  13.7's note): the "How this works" disclosure and the always-visible
  privacy lines (step 11.5), and "Reset everything" — now a full wipe of
  every section/assessment/record, standing in for the real, scoped
  Phase D purge until that lands.
- **13.5 Section create/edit, done** as `SectionForm.tsx`. Course code,
  section label, semester, ID digits — plus Phase B's roster upload,
  built in the same pass (see below), so "no roster yet" never shipped as
  a real, usable state.
- **13.6 Assessment create, done** as `AssessmentForm.tsx` — Setup.tsx's
  old form minus ID digits (inherited from the section, never asked
  twice) minus workbook mode, `validateConfig.ts` reused unchanged.
- **13.7 `App.tsx`'s screen enum, done**: `library | section | assessment
  | scan | results`. **Setup.tsx is deleted outright**, not kept
  alongside the new screens — its role is fully absorbed by
  Library/SectionForm/AssessmentForm, and the step's own screen-enum list
  has no `'setup'` in it. Every screen has a visible way back (`All
  sections` on Scan and Results; `Cancel` on both forms).
- **13.8 Scoped reads, done.** `getRecordsByAssessment(assessmentId)`
  replaces the unscoped read in both Results' table and Scan's saved
  counter. `getAllRecords()` stays as a primitive (used by the v5
  migration itself, and reserved for Phase D's purge-preview counts) —
  nothing else calls it any more.

**Done when (Phase A):** a second quiz genuinely does not destroy the
first — verified by `sections.test.ts`/`db.test.ts`'s scoped-query and
migration cases, and by `Library.test.tsx` rendering the exact four-
section semester from the design discussion (CSE100, CSE200, two CSE203
sections) without one section's assessments leaking into another's count.

#### Phase B — the roster belongs to the section — **DONE 2026-09-10**

Built in the same pass as Phase A rather than after it — see Phase A's
own note on why shipping Phase A alone would have been a real regression.

- **13.9 Roster moved onto `Section`, done.** `SectionForm.tsx`'s upload/
  parse/confirm-card/picker UI is a straight port of Setup.tsx's old
  workbook-mode JSX (step.md 12.1-12.4) — relocated, not rewritten, same
  as 2r.0 moved the recognizers. The old `rosterUpload` IndexedDB store is
  retired at the v5 migration (13.2), its data folded into the migrated
  Section. **One deliberate design simplification beyond the original
  spec**: there is no "plain vs. workbook" mode toggle on this screen the
  way Setup.tsx had one. A section either has a roster attached or it
  doesn't — an always-visible, optional upload field — because the mode
  choice only ever made sense as a *per-quiz* decision when the roster was
  re-uploaded every quiz; now that it is a property of the section, "plain
  download" isn't a mode to pick, it's just what happens when nothing was
  attached. Results.tsx's plain button is exactly as unconditional as it
  always was.
- **13.10 Every assessment inherits its section's roster, done.** Scan's
  "Scanned 7 of 16", Review's name/suggestion, Results' Name column — all
  read `section.roster ?? null` now; `App.tsx` is the only place that
  reads it off `activeSection`, and no assessment-creation flow asks for
  a file.
- **13.11 Provenance, done.** Every workbook export shows "Working from
  the file you picked / the copy this app wrote on `<date>`", plus a
  **Re-pick** button. Re-pick does not duplicate a second file-picker UI —
  it reopens the section's own edit screen (`SectionForm`, which already
  has the upload control), routed back to Results afterward rather than
  bouncing to the library and losing the open assessment (`App.tsx`'s
  `sectionFormReturnsTo` state exists specifically for this one case).
- **13.12 Re-cache, done — and verified as the actual correctness
  property it exists for, not just that the field changed.** After a
  successful workbook export, the bytes just written become the section's
  new `workbook.bytes` (`source: 'exported'`), via `onSectionUpdated`
  threading back up to `App.tsx`'s `activeSection`. The test that matters
  is `Results.test.tsx`'s "a second quiz in the same section keeps the
  first quiz's sheet" — it exports Quiz 1, re-renders `Results` with the
  RE-CACHED section and a Quiz 2 assessment (exactly what `App.tsx`
  produces in the real app), exports again, captures the real
  `Blob` (not just the mocked URL), reloads it as a genuine ExcelJS
  workbook, and asserts both `Quiz 1` and `Quiz 2` sheets are present
  alongside the class list. Without 13.12 this is exactly the bug the
  whole section exists to prevent: a later quiz reloading from the
  pristine original upload and silently dropping every earlier quiz's
  sheet from its own download.

**A third thing landed alongside A and B, ahead of its own phase**:
**13.20 (stamp `exportedAt`)** was pulled forward from Phase D and built
now, via `Results.tsx`'s `markExported()` — called after both the plain
download and a successful workbook export. Reasoning: `exportedAt` is
otherwise dead weight until Phase D's purge guard reads it, and every
assessment created between now and whenever Phase D lands would need a
retroactive backfill (or worse, look purge-eligible when it never really
was exported) if this waited. **13.21 (the disclosure wording)** is also
already satisfied, not deferred: `Library.tsx`'s roster-disclosure line
was written fresh as "for as long as that section exists," never carrying
the old "for the session" wording that would have needed correcting
later — there was no stale sentence to catch this time, because nothing
here was edited from Setup.tsx's old copy, it was written new.

A fourth thing landed beyond the letter of the spec: **13.13's persistent
context header was extended to `Review.tsx` itself**, not just Scan and
Results. Review renders as a `position: fixed` overlay covering Scan's
own header entirely (see CLAUDE.md's "Things to avoid" on why it has to
render that way), which means the section context Scan shows underneath
is invisible at the exact moment the instructor is deciding whether to
Confirm a save — the highest-value place for it to be visible, not the
lowest. Fixed with an optional `sectionLabel` prop and a bare `.eyebrow`
CSS rule (previously scoped to `.app-header .eyebrow` only).

Both phases: 33 backend-untouched, frontend-only files changed; suite
grew 245 → 282 (`sections.test.ts`, `Library.test.tsx`,
`SectionForm.test.tsx`, `AssessmentForm.test.tsx` all new;
`db.test.ts`/`Results.test.tsx`/`Review.test.tsx` rewritten for the new
schema and props; `Setup.test.tsx` deleted with `Setup.tsx`). `npm run
build` and three consecutive full `vitest run` passes confirm no
flakiness; one real timing bug was caught and fixed along the way (two
`SectionForm.test.tsx` cases clicked Submit before the mount-time
`getAllSections()` fetch had resolved — passed by luck in isolation,
failed under the full suite's different scheduling; fixed by giving the
submit button a real `disabled` state tied to the fetch, not just by
patching the tests).

#### Phase C — not grading into the wrong section — **DONE 2026-09-10**

The phase that exists for one failure: thirty scripts saved against
CSE203-1 when the class in the room is CSE203-2. It is silent and it looks
exactly like success.

- **13.13, done** — see Phase B's fourth item above; landed early because
  it was cheap once Scan/Results/Review already needed a `sectionLabel`.
- **13.14 Resume confirmation, done.** `sections.ts` gained two pure
  functions: `lastActivityAt(capturedAtValues)` (the latest of a list, or
  `null` for none — DERIVED from records `Library.tsx` already fetches to
  show scanned counts, not a stored field with its own sync problem) and
  `needsResumeConfirmation(lastActivityAt, now)`, which compares by LOCAL
  calendar day rather than a rolling 24-hour window — an assessment
  scanned at 11pm and reopened at 7am is a different session even though
  under 24 hours passed. A brand-new assessment (`lastActivityAt === null`)
  never confirms — nothing to lose by opening it, and it's the common
  case, so no tap is added there. `Library.tsx` shows a banner ("Resume
  CSE203-2 · Quiz 1 — last scanned `<date>`?") only when it's earned.
  **One real trap caught while writing the test fixtures**: an initial
  version hardcoded UTC timestamps a minute before midnight to represent
  "yesterday," which is the SAME local calendar day as "now" in any
  timezone ahead of UTC — the test failed on this machine's own timezone,
  correctly, before it could ship as a false negative. Fixed by deriving
  every fixture from `now` via plain millisecond arithmetic (`now - N
  days`) instead of a hand-picked UTC string, which is safe in any
  timezone the test actually runs in.
- **13.15 Scope the duplicate checks, done.** `findRecordsBySerial`/
  `findRecordsByStudentId` take an `assessmentId` and filter the index
  lookup's result down to it — an in-memory filter after the existing
  index read, not a second index, since the result set is a handful of
  rows at this app's real scale. `Review.tsx`'s `handleConfirm` passes
  `assessmentId` through (already in scope). Verified two ways: `db.test
  .ts` proves the same serial in two different assessments no longer
  cross-matches; `Review.test.tsx` proves the identity-conflict BANNER
  itself doesn't appear for a same-serial record in a different
  assessment, while still appearing for one in the SAME assessment — the
  second case exists specifically so scoping the query couldn't
  accidentally break the check it was never meant to touch.
- **13.16 Filename carries the identity, done.** `exportFilename` now
  takes the `Section` alongside the quiz name and builds
  `CSE203-2_Quiz-1_2026-09-09.xlsx` — course-label, quiz name (spaces to
  hyphens), ISO date, joined by underscores, reusing the same
  character-sanitiser the quiz name alone used before (issues.md N7),
  now applied to the section part too. Verified by spying on
  `HTMLAnchorElement.prototype.click` and reading the real `.download`
  attribute `triggerDownload` sets, rather than trusting the mocked
  `URL.createObjectURL` call happened at all.

Frontend suite: 282 → 301, four consecutive full `vitest run` passes
confirmed stable. No backend changes.

#### Phase D — the semester boundary — **DONE 2026-09-10**

- **13.17 Semester on `Section` — done**, landed as part of Phase A:
  `groupSections` groups by it and `Library.tsx` renders the grouping.
  **Still not done**: "current semester first" — `groupSections`' own
  comment is explicit that it uses insertion order, not a real notion of
  "current." This is unaffected by 13.22's picker decision below: a
  canonical `"Season YYYY"` string doesn't by itself tell `groupSections`
  which one is "now," so ordering by real recency, if ever wanted, is
  still a separate, unbuilt decision. The purge below sidesteps needing
  it at all — see 13.18's own note on why.
- **13.18 The purge flow, done.** Triggered exactly where plan.md §18
  specifies — "creating a section under a new label" — never on a timer,
  never automatic, and never merely from revisiting a multi-semester
  library. `App.tsx`'s `SectionForm` save handler reads every existing
  section's semester **before** saving (skipped entirely for an edit,
  never just a semester-field change — the spec's own wording is
  "creating," and editing an existing section into a different semester
  is deliberately out of scope), and only when the just-saved semester
  matches none of them AND at least one prior section already existed
  (nothing to purge on the very first section ever) does it hand
  `Library.tsx` a one-shot `pendingSemesterOffer`. This sidesteps the
  undecided "which semester is current" question in 13.17 entirely — the
  trigger only needs to know "this exact label didn't exist a moment
  ago," never which of several existing labels is the "current" one.
  `Library.tsx` shows one card per OTHER semester (`sections.ts`'s new
  `otherSemesters`, `purgePreview`), each with real section/assessment/
  record counts, a **Review** button, and a session-only **Not now** that
  never resurfaces the offer once dismissed (nor once a semester is
  successfully purged) — but never persists past this signal, so a
  genuine app reload re-derives whether it's still relevant rather than
  remembering a stale dismissal forever.

  **Deliberately, per the label-drift risk plan.md §18 itself names**:
  `otherSemesters`/`purgePreview` compare semester labels by EXACT string,
  never case/whitespace-normalized. "Fall 2026" and "fall 2026" are
  offered as two SEPARATE purge candidates, not silently folded into one
  — a normalized match would have hidden exactly the drift the open risk
  warns about, inside the one feature that deletes data.
- **13.19 The unexported guard, done**, as its own `PurgePreview.blockedBy`
  field: any assessment with `exportedAt === null` blocks that semester's
  purge outright, named individually (section + quiz name), Cancel only —
  the same block-then-confirm shape `Results.tsx`'s duplicate-scan panel
  already uses. `exportedAt` had already been stamped since Phase B
  (13.20, pulled forward), so this guard has real data to read from day
  one rather than a semester of retroactive nulls.
- **13.20, done early** — see Phase B.
- **13.21, done early, and already satisfied** — see Phase B.
- **13.22 Semester picker, and opening on the library — DONE 2026-09-10,
  as a direct follow-up once the user picked between the two things named
  "genuinely unsettled" below.** Two decisions, both frontend-only:

  1. **Semester labels are now a picker**, not free text. `sections.ts`
     gained `SEMESTER_SEASONS` (`Spring`, `Summer`, `Autumn` — three
     terms, matching the pilot institution's own trimester calendar, no
     Winter), `formatSemesterLabel`/`parseSemesterLabel` (the one place a
     season+year pair becomes, or is recovered from, the plain string
     every other function in the module already operates on), and
     `currentSemesterSeason` (today's own season, used only as a
     brand-new section's starting point). `SectionForm.tsx`'s free-text
     `<input id="semester">` is replaced with three season buttons plus a
     year number field, using the same `NumField` clearable-input
     discipline `idDigits` already had (an empty year stays empty, never
     writes back a `0`). **Editing a pre-picker section** (`semester`
     something like `"Fall 2026"` or `"F26"`, neither of which
     `parseSemesterLabel` accepts) falls back to `currentSemesterSeason()`
     / today's year rather than crashing or leaving the picker blank —
     nothing is overwritten until Save is actually tapped, so the fallback
     costs at most one extra tap to correct, never silent data loss.
     This closes 13.17/13.18's own "free text vs. picker" question for
     all NEW data outright — the picker cannot produce anything but a
     canonical `"Season YYYY"` string. It does not retroactively fix
     legacy labels, which is why 13.18's purge still compares by exact
     string rather than normalizing: that comparison was always there for
     drift the picker cannot reach into the past to prevent, not drift
     the picker itself could still create.
  2. **The app opens on the library, confirmed rather than newly built.**
     `App.tsx`'s default `Screen` state and its final fallback render
     already resolved to `Library` before this — verified, not changed —
     so this half of the task was a documentation decision (plan.md §18,
     this file's own Progress-table row and the "genuinely unsettled"
     paragraph below), not a code change.

  `App.test.tsx`'s `fillAndSubmitNewSection` helper (and its one
  direct-edit test) needed updating to drive the new picker — click a
  season button, set the year field by its `aria-label` — since it
  previously drove the old free-text input directly; every assertion
  that checked a stored label was updated from `'Fall 2026'` to
  `'Autumn 2026'` at the same time, since `'Fall'` is no longer a
  producible option. 6 new `sections.test.ts` cases
  (`formatSemesterLabel`/`parseSemesterLabel` round-trip, the unparseable
  case, all three `currentSemesterSeason` month boundaries) and 5 new
  `SectionForm.test.tsx` cases (picker selection landing in the saved
  string, the unparseable-legacy-value fallback, round-tripping a
  picker-produced value back through the picker on edit, and the cleared/
  empty-year and non-4-digit-year cases mirroring `idDigits`' own
  describe block). Frontend suite: 322 → 333, three consecutive full
  `vitest run` passes confirmed stable. `npm run build` and backend
  `pytest` (256, unaffected) both re-run clean; the design-skill AI-tell
  scanner stayed clean on the new picker UI. No backend changes.
- **13.23 Delete a single section — DONE 2026-09-10**, requested
  directly: "if someone cleared a section wrongly," they need a way to
  remove it, distinct from the semester purge (13.18, a whole semester)
  and "Reset everything" (a full device wipe) — both too blunt for one
  mis-created section. Reuses `db.ts`'s existing `deleteSection()` cascade
  primitive (built ahead of Phase D, 13.1, and already covered by
  `db.test.ts`'s cascade suite) rather than adding a second one — a
  section deleted this way is removed exactly as thoroughly as one purged
  by semester: itself, plus every assessment under it, plus every record
  under those. `sections.ts` gained `sectionDeletePreview`, the
  single-section counterpart to `purgePreview` (13.18) — same shape, same
  fields, scoped to one section instead of a whole semester. `Library.tsx`
  gained a **Delete** button on every section card, next to Edit and
  + New assessment, and `SectionDeleteReviewPanel` — the same
  block-then-confirm shape `PurgeReviewPanel` already uses: an assessment
  with `exportedAt === null` blocks the delete outright, named
  individually, Cancel only; otherwise a plain confirm stating exactly
  what would go (assessment and record counts), no undo, Cancel available
  right up to the destructive tap itself. Deliberately reuses the
  unexported-assessment guard even though the user's own stated case (a
  wrongly-created section) usually has nothing to lose — a section that
  *does* hold ungraded work is exactly as unrecoverable deleted singly as
  it would be deleted via a semester purge, and there's no reason this
  smaller-blast-radius path should be less careful than the bigger one.
  6 new `Library.test.tsx` cases: immediate delete of an empty section
  behind the confirm step, the confirm step itself adding no deletion on
  its own, Cancel leaving the section untouched, the cascade to
  assessments and records, a sibling section surviving untouched, and the
  unexported-assessment block naming the quiz with Cancel-only. Frontend
  suite: 333 → 339, three consecutive full `vitest run` passes confirmed
  stable. `npm run build` and the AI-tell scanner both re-run clean. No
  backend changes — this is entirely `sections.ts`/`Library.tsx`, reusing
  a store primitive that already existed.
- **13.24 Delete a single assessment, and typed confirmation on both
  delete flows — DONE 2026-09-10**, requested directly as a follow-up to
  13.23: "wrongfully added assessments" need the same escape hatch a
  wrongfully-added section already has, and both destructive deletes
  should make the instructor type the name back rather than trust one
  tap. Two changes, landed together since the second touches the first's
  own UI:

  1. **Assessment deletion.** `db.ts` gained `deleteAssessment()` — one
     quiz and every record under it, leaving the section and every
     sibling quiz untouched (narrower than `deleteSection()`, which this
     does not call). `sections.ts` gained `assessmentDeletePreview`, the
     one-level-narrower counterpart to `sectionDeletePreview`. A
     **Delete quiz** button on every assessment row in `Library.tsx`
     opens `AssessmentDeleteReviewPanel`, the same block-then-confirm
     shape every other delete flow in this app now uses.
  2. **Typed confirmation**, on the section and assessment delete flows
     specifically (not the semester purge or "Reset everything," neither
     of which were asked for). A new shared `TypedDeleteConfirm`
     component replaces the old single "Yes, delete X" button: the
     instructor must type the exact name — `CSE100-1` for a section,
     `Quiz 1` for an assessment — into a field before the confirm button
     enables at all. Trimmed but otherwise exact, deliberately: the point
     is confirming they read and typed the real name, not guessing
     whether they roughly meant it. Placeholder text doubles as the
     expected value, both as a visible hint and as a stable thing for
     tests to target.

  **A real correctness gap was caught and fixed while building this**,
  not after: `sectionDeletePreview`/`purgePreview`'s existing
  unexported-assessment guard blocked on `exportedAt === null` ALONE,
  with no regard to whether the assessment held any records. A brand-new,
  wrongly-added assessment is *also* always unexported — so the guard, as
  it stood, would have made it impossible to ever delete the exact thing
  step 13.24 exists to let go of, the moment that section held even one
  empty assessment. Fixed by narrowing the guard everywhere it's used
  (`isBlocking`, `sections.ts`) to `exportedAt === null AND recordCount >
  0` — there is nothing to lose deleting an empty, never-exported
  assessment, and the guard is for real student work, not paperwork. This
  changes `purgePreview`'s and `sectionDeletePreview`'s established
  behaviour, not just adds `assessmentDeletePreview`'s: two existing
  `Library.test.tsx` cases (the semester-purge block, the section-delete
  block) needed a real saved record added to their previously-empty
  "unexported" fixture assessment to keep demonstrating the block
  correctly, and two new tests pin the boundary directly — an unexported,
  EMPTY assessment does NOT block either a section delete or a semester
  purge.

  17 new tests: `db.test.ts` (+1, `deleteAssessment`'s cascade),
  `sections.test.ts` (+9 — `sectionDeletePreview`/`assessmentDeletePreview`
  direct coverage neither had before, plus the purge/section-delete
  empty-assessment boundary cases), `Library.test.tsx` (+7 net — the
  typed-confirm gate itself, deletion, cascade, sibling-untouched and
  block cases for assessments, plus the empty-assessment non-blocking
  case for both delete flows). Frontend suite: 339 → 358, three
  consecutive full `vitest run` passes confirmed stable. `npm run build`,
  `tsc -p tsconfig.app.json --noEmit`, and the AI-tell scanner all re-run
  clean. Backend `pytest` (259) re-run as a sanity check, unaffected — no
  backend changes.

Frontend suite: 301 → 322 (18 new `sections.ts` cases for
`otherSemesters`/`purgePreview`, 8 new `Library.test.tsx` cases for the
purge UI itself, 5 new cases in a first-ever `App.test.tsx` covering the
new-semester DETECTION specifically — the one piece of this phase that
isn't reachable by testing Library in isolation with the prop handed in
directly). Five consecutive full `vitest run` passes confirmed stable.
No backend changes.

### Test

Per layer, as CLAUDE.md's testing rule requires — and note that most of
this step is pure logic and storage, which means most of it is genuinely
unit-testable rather than requiring a device:

- **`sections.test.ts`, done** — 14 cases: grouping, sorting, display
  labels, duplicate section detection, `assessmentConfig`. Pure, no DOM.
- **`db.test.ts`, done.** The v4 → v5 migration as its own describe
  block: a populated v1 database (config + records) becomes one Section
  and one Assessment with every record preserved; an empty v1-v4 database
  produces nothing; a v4 database's persisted roster folds into the
  migrated Section; a v5 database is left alone (no double migration);
  `meta`/`getSourceId()` verified untouched across a v1 jump. Plus a
  separate `deleteSection` cascade suite (assessments and records under
  it go too, a sibling section is untouched) — written ahead of Phase D's
  own purge UI, since the store-level primitive `deleteSection()` was
  built now.
- **Scoped queries, done, including the conflict-check scoping.**
  `getRecordsByAssessment` — two assessments, the same-shaped records in
  each, only the right ones come back. `findRecordsBySerial`/
  `findRecordsByStudentId` (13.15) — a shared serial or student ID across
  two different assessments no longer cross-matches, and a genuine match
  WITHIN the same assessment still does (`db.test.ts`); `Review.test.tsx`
  proves the same pair of outcomes at the UI level, on the actual
  conflict banner rather than just the query.
- **`sections.test.ts` — `lastActivityAt`/`needsResumeConfirmation`,
  done** — 8 more cases beyond Phase A/B's 14: latest-of-several,
  same-day (no confirm), days-ago and weeks-ago (confirm), and same
  local calendar day at different hours (no confirm) — the fixture that
  initially failed on this machine's own timezone until it stopped
  hardcoding UTC-adjacent strings (see Phase C's own write-up above).
- **`Library.test.tsx`, done** — 15 cases: the four-section semester from
  the design discussion (CSE100, CSE200, two CSE203 sections) rendering
  as three course groups with CSE203 carrying two; an assessment's
  scanned count and exported/not-exported badge; the disclosure and
  Reset-everything suites ported from the old `Setup.test.tsx`; and 4
  more for 13.14's resume confirmation — no confirm for a same-day scan,
  a confirm banner for one days old, Resume proceeding and clearing the
  banner, Cancel leaving the library untouched.
- **`SectionForm.test.tsx`, done** — 18 cases: the duplicate-section
  block (and that it does NOT fire across different semesters), pre-fill
  and self-exclusion when editing, and the full roster-upload suite
  ported from `Setup.test.tsx`'s old workbook-mode tests (parse, ambiguous
  picker, corrupt file, duplicate IDs, re-normalizing on an idDigits
  change, keeping vs. removing an existing roster on edit).
- **`AssessmentForm.test.tsx`, done** — 8 cases: the array-resize guard
  (issues.md #1), the cleared-number-field fix ported for the fields that
  now live here, `idDigits` correctly absent from what's handed to
  `onSave`.
- **Purge suite, done.** `sections.test.ts` — `otherSemesters` (excludes
  the current semester, deduplicates, and keeps differently-cased labels
  as distinct entries on purpose) and `purgePreview` (per-semester
  counts scoped to THAT semester only, not leaking another semester's
  numbers; the block list names the right assessment; an empty or
  assessment-less semester previews as zero, not an error). `Library
  .test.tsx` — no offer when nothing is pending or only one semester
  exists; real counts shown; an unexported assessment blocks with
  Cancel-only and nothing deleted; a confirmed purge deletes sections,
  assessments AND records while the new semester survives, and the offer
  clears itself; Cancel from the confirm view returns to the list
  without deleting; "Not now" dismisses without deleting; two
  differently-cased labels offered as separate candidates. `App.test
  .tsx` (new file) — the detection itself: no offer for the first-ever
  section, none for a second section in the same semester, a real offer
  for a genuinely new one, the offer not resurfacing on an unrelated
  later visit, and an EDIT that changes a section's semester field
  deliberately not counting (13.18's own scope: "creating," not
  "editing into").
- **Existing suites re-run, with additive changes only.** `Review.test
  .tsx`/`Results.test.tsx` needed `assessmentId` added to every render
  (a genuinely required field, not optional functionality — see step
  13's own note on why this is a real exception to "additive and
  defaulted") and `Results.test.tsx`'s `rosterUpload`/`onReset` props
  were replaced with `section`/`assessment`/the new callbacks, since
  those props were retired, not merely extended. `roster.ts`,
  `examSheet.ts`, `workbookExport.ts`, `rosterMatch.ts`,
  `validateConfig.ts` and `scanQueue.ts` themselves needed **zero**
  changes — exactly the "behaviour this step does not change" signal the
  spec asked to watch for. Frontend suite: 245 → 282 (Phase A/B) → 301
  (Phase C) → 322 (Phase D).
- **By hand, on the real device — still needed, not yet done.** Two
  sections graded in one sitting without a reset, the context header
  actually read under time pressure, a workbook export into a section's
  file that already holds a previous quiz's sheet, and now a real
  end-of-semester purge on real IndexedDB data. The in-memory versions of
  the re-export and the purge ARE both verified (Phase B's real-Blob
  re-export test; Phase D's own purge suite deleting real sections,
  assessments and records and confirming the survivors) — what's still
  missing is a real phone, a real file picker, a real re-upload to Drive,
  and a real multi-semester device to purge.

### Done when

Both CSE203 sections can be graded in the same afternoon, one after the
other, with no reset between them and both sets of marks still on the
phone afterwards. A section's class list is picked once and every later
quiz in that section uses it without asking again. Last week's quiz is
still there to look at, and re-exportable. Every workbook export says
which copy of the file it is writing into and offers to re-pick it. The
scan loop is unchanged — same taps, same screens, same timing. A student
ID shared between two courses raises no conflict. An upgrade from v4 with
a session in progress loses nothing. And the semester purge refuses to
run while any assessment has never been exported, naming it.

**Every clause in this paragraph is now met, as of Phase D, 2026-09-10.**
The last one — the semester purge refuses to run while any assessment has
never been exported, naming it — is `PurgePreview.blockedBy` and
`Library.tsx`'s `PurgeReviewPanel`: a real assessment with `exportedAt
=== null` blocks that semester's purge outright, named by section and
quiz, Cancel only, verified by a test that then confirms nothing was
deleted. Grading two sections in one afternoon, the class list picked
once, last week's quiz still there and re-exportable, provenance and
Re-pick on every workbook export, the scan loop unchanged, an upgrade
from v4 folding an in-flight session rather than dropping it, a student ID
shared between two courses raising no conflict — all verified by the
suites named in Phase A/B/C/D's own write-ups above, not just asserted
here. Frontend suite: 245 → 282 → 301 → 322 across the four phases, then
322 → 333 for 13.22's picker/library-opening follow-up, 333 → 339 for
13.23's per-section delete, and 339 → 358 for 13.24's per-assessment
delete plus typed confirmation on both. Backend untouched throughout —
no API change, no new dependency, exactly as rule 2 required from the
start.

**Both things named as genuinely unsettled in plan.md §18 are now
decided, as of 13.22 (2026-09-10)**: semester labels are a picker
(Spring/Summer/Autumn plus a year — see 13.22 above for why 13.18's purge
still keeps its exact-string comparison rather than dropping it now that
new drift can't occur), and the app opens on the library, confirmed
rather than the jump-straight-into-the-last-assessment alternative. The
picker doesn't retroactively fix a pre-picker label like `"F26"` — that
class of drift is exactly what 13.18's exact-string purge comparison
still exists to surface, now narrowed to legacy data only.

---

## Step 14 — Landing page

**Goal.** Someone who was sent the link understands what this is, how it
works, and what it refuses to do, before they ever see "Your sections" —
and an instructor who already knows never has to scroll past it again.

Specced 2026-09-10; plan.md §19 carries the fuller rationale, including
why this is pre-rendered rather than server-rendered despite the request
for the latter. Same relationship to §19 that step 13 has to §18. A
deliberate extension beyond plan §13's MVP scope, and like every
extension before it: the capture → review → confirm loop is untouched,
and **there are no backend changes at all**.

### Before you start

Read `plan.md` §19 end to end (it decides more than this file does),
`frontend/src/index.css`'s `:root` and `prefers-color-scheme: dark`
blocks (the landing ladder extends those values, it does not replace
them), `frontend/src/App.tsx`'s `Screen` enum, and `frontend/vite.config.ts`
(the PWA config the prerendered document has to stay compatible with).

### Rules this step is bound by

1. **No backend changes.** Not one. If a substep seems to need one, it is
   the wrong substep.
2. **No new runtime dependency.** `renderToStaticMarkup` comes from
   `react-dom`, already a dependency. The animation is CSS. The fonts are
   the system stack.
3. **The weight budget in §19 is a bar, not an aspiration** — under
   ~10KB gzipped for a complete first paint. A change that breaks it is
   the thing that is wrong.
4. **`prefers-reduced-motion: reduce` collapses the animation to its
   final state.** Non-negotiable, and it ships in the same phase the
   animation does — never "added after".
5. **Landing tokens are scoped to the landing page.** Nothing may leak
   into `:root` or the app's own theming breaks.
6. **Mobile-first, authored from the phone up.** Base styles are the
   phone styles; every media query is `min-width`. No horizontal overflow
   at 320px, and nothing depends on `100vh` (iOS Safari changes it
   mid-scroll as the address bar collapses — `#root` already uses
   `100svh` for exactly this reason). This repo has shipped three
   desktop-fine/phone-broken layouts already; this rule is why.

### Phase A — the page, rendered client-side — **DONE 2026-09-10**

Shippable on its own: the story exists and can be read, reviewed and
iterated before any build plumbing changes.

- **14.1 Landing tokens — done**, as `frontend/src/landing.css`. A dark
  palette scoped entirely under a `.landing` selector — every `--lp-*`
  custom property is declared there, never on `:root`, so the app's own
  `prefers-color-scheme` theming is provably unaffected. The four-step
  surface ladder reuses the app's dark values verbatim (`--lp-bg` /
  `--lp-surface` / `--lp-surface-2` equal index.css's dark
  `--background`/`--surface`/`--surface-2`) plus one new, deeper
  `--lp-canvas` step for the sticky top bar. Hairline borders throughout,
  no drop shadows. The display ramp is `clamp()`-based and phone-rooted
  (~32px → 64px for the hero, ~28px → 40px for section heads), and the
  section rhythm is a custom property (`--lp-section-y`, 48px base) grown
  via two `min-width` queries to 64px and 96px — never a desktop number
  shrunk down.

  **One thing the tokens had to solve that wasn't obvious in the spec**:
  the app's own `.btn-primary`/`.btn-secondary`/`.btn:focus-visible`
  classes read the app's theme-aware `--primary`/`--surface` tokens,
  which on a light-mode system resolve to LIGHT colors — wrong on a page
  that's dark unconditionally (decision 2). Rather than duplicating the
  whole button system, `.landing .btn-primary` etc. override only the
  color properties, scoped under `.landing`, and keep every geometry rule
  (pill shape, 44/36px height, hover/active/disabled mechanics) from the
  shared class untouched. One reusable button system, two color sources,
  correct regardless of the reader's own OS theme.

- **14.2 `Landing.tsx` — done.** The seven sections from §19, in order,
  plus the pinned top bar (`position: sticky`, not `fixed` — same visual
  result for a single scrolling page without `fixed`'s extra
  top-padding bookkeeping). The phone hero is headline + one line + CTA,
  with `ScanGraphic` (a new, separate component) peeking from the bottom
  edge of a height-bounded container; the two-column arrangement arrives
  at `min-width: 64em`. `ScanGraphic` ships now as a static illustration
  of the animation's final, settled state — the ID row, four question
  cells, one of them showing a flag instead of a number — built as
  distinct, separately-styled SVG groups specifically so Phase C
  (14.7/14.8) can animate this same markup rather than replace it.
  Purely presentational: no hooks anywhere in `Landing.tsx`, so 14.5's
  eventual `renderToStaticMarkup` call has nothing to trip over.
- **14.3 The copy — done.** Every factual claim in sections 4 and 5 is
  pinned by its own `Landing.test.tsx` assertion, checked against what
  the claim actually maps to: "never filled in with a confident guess"
  (the CNN decoder's floor-and-flag behaviour), "never writes one
  question's mark into another's column" (`column_count_mismatch`
  failing the whole scan), "a blank never exports as a zero"
  (`examSheet.ts`'s own rule), "nothing leaves that laptop... no third
  party ever sees a script" (the default `cnn` recognizer, worded to
  match CLAUDE.md's own correction of the stronger, false "never leaves
  the device" claim), "no account, no server-side database" (still true
  everywhere in this repo).
- **14.4 Entry and exit — done.** A new pure module, `landing.ts`
  (`hasSeenLanding`/`markLandingSeen`), reads and writes a single
  `localStorage` key — not the async IndexedDB `meta` store, which would
  paint the landing page and then yank it away once the read resolved.
  Both functions fail toward SHOWING the landing page (read throws ->
  `false`) or toward doing nothing destructive (write throws -> silently
  no-ops) rather than crashing, since Safari private browsing and some
  embedded webviews make `localStorage` throw rather than simply being
  absent. `App.tsx`'s `Screen` union gains `'landing'`; its initial state
  is computed synchronously from `hasSeenLanding()`. **The way back**:
  `Library.tsx` gains an optional `onShowLanding` prop and a "Read the
  full story" button inside its existing "How this works" disclosure —
  optional so no other render call or test needed to change, and every
  existing `Library.test.tsx` case stayed valid unmodified.

  25 new tests: `landing.test.ts` (6 — both functions, including the
  storage-throws cases), `Landing.test.tsx` (13 — structure, the pinned
  CTA present in at least 3 places and all wired to the same handler,
  and one assertion per factual claim), `App.test.tsx` (4 — first visit
  shows the landing, a dismissed visitor skips it, "Open the app" marks
  it seen, the way-back round trip), `Library.test.tsx` (2 — the button
  calls through when provided, is absent when it isn't). Frontend suite:
  358 → 383, three consecutive full `vitest run` passes confirmed stable.
  `tsc -p tsconfig.app.json --noEmit`, `npm run build`, and the AI-tell
  scanner all clean. Backend `pytest` re-run as a sanity check (259,
  unaffected — no backend changes, per this step's own rule 1).

  **Not yet true, on purpose — Phase B is what fixes it**: the built
  bundle currently grew (CSS 10.42KB → 14.05KB gzip 2.89→3.59KB; main JS
  244.68KB → 252.26KB gzip 75.67→77.70KB), because Phase A still ships
  `Landing.tsx` as an ordinary eagerly-imported screen in the React tree,
  exactly like every other screen. §19's weight budget (under ~10KB
  gzipped for a complete first paint) is a claim about the *final*,
  prerendered, zero-runtime-JS shape Phase B produces — not about this
  intermediate state, where the landing page still costs its full React
  weight like anything else in the bundle. Recorded here so the number
  isn't mistaken for a regression against the wrong bar.

### Phase B — static-first delivery — **DONE 2026-09-10**

The performance half. Nothing a reader sees changes; what changes is when
they see it.

- **14.5 The prerender step — done**, as `frontend/scripts/prerender-
  landing.mjs`, wired into `npm run build`
  (`tsc -b && vite build && node scripts/prerender-landing.mjs`) so there
  is no second command to forget. It bundles a new tiny entry point,
  `src/prerenderEntry.tsx`, to plain JS via **Vite's own `build()` API in
  SSR mode** — reusing the bundler already doing this exact JSX/TS
  transform for the real app, rather than adding a second toolchain, is
  what keeps this step's "no new dependency" rule intact. That bundle is
  then imported in Node and its one export, `renderLandingMarkup()`
  (`react-dom/server`'s `renderToStaticMarkup`, actually running),
  produces the HTML. `landing.css` is read as raw text separately — an
  SSR build silently drops CSS imports rather than bundling them, which
  turns out to be exactly what's wanted, since nothing in the client
  bundle should carry this page's styles any more (see 14.6). The script
  injects the markup, that CSS, and 14.6's bootstrap script into the real
  `dist/index.html`, then deletes its own throwaway SSR output
  (`dist-ssr/`, reusing Vite's own conventional name so no new
  `.gitignore` line was needed).
- **14.6 Zero runtime JS for the landing — done.** The markup moves
  *outside* `#root` (a sibling `<div id="landing-root">`), so React never
  owns it, hydrates it, or re-renders it. Visibility is one class,
  `ms-app-visible`, added to `<html>` by a ~15-line inline bootstrap
  script reading the **synchronous `localStorage` flag** `landing.ts`
  already defined — not the async IndexedDB `meta` store, which would
  paint the page and then yank it away — and failing toward SHOWING THE
  APP (never a blank page) if that read throws. The CTA works before the
  main bundle arrives because the same script attaches its click handler
  to `document` itself via `closest('[data-open-app]')` — event
  delegation, since the script runs from `<head>`, before the buttons in
  `<body>` even exist yet to attach a direct listener to. `landing.css`
  gains the two-rule toggle itself: `html.ms-app-visible #landing-root {
  display: none }` and `html:not(.ms-app-visible) #root { display: none
  }` — deliberately never a `display: block/flex` rule for the VISIBLE
  case, since `index.css` already owns `#root`'s real layout
  (`display: flex`) and a same-or-higher-specificity override would
  silently have clobbered it the moment the class was added.

  **What this retired, on purpose.** `App.tsx`'s `Screen` union drops
  `'landing'` entirely — the app now always starts on the library, since
  the pre-load decision is the static shell's job, not React's.
  Phase A's `hasSeenLanding`/`markLandingSeen` functions are deleted:
  nothing calls them any more (the bootstrap script above IS that check
  now, hand-transcribed to plain JS because it must run before any bundle
  exists to import TypeScript from). What survives from `landing.ts`:
  `LANDING_SEEN_KEY` and a new `APP_VISIBLE_CLASS`, both re-exported
  through `prerenderEntry.tsx` so the Node script embeds the exact same
  literals rather than a hand-typed second copy that could drift. The
  "way back" (`Library.tsx`'s `onShowLanding` prop, unchanged) no longer
  needs a client-rendered `<Landing>` to switch to — `showLandingOverlay()`
  just removes the same class the bootstrap script adds, revealing the
  identical static markup that's been sitting in the DOM the whole visit,
  which is why `Library.tsx` and `Library.test.tsx` needed zero changes
  for this phase.

  **Guarding the failure mode this step names as a named risk**: a
  prerender step can silently stop injecting anything while every
  component-level test keeps passing, since `Landing.tsx` itself never
  changed. `prerender.test.ts` runs the real `npm run build` (the one
  deliberately expensive test in this suite) and reads the actual
  `dist/index.html`, checking the markup, the bootstrap script and the
  critical CSS are genuinely present, keyed to the real
  `LANDING_SEEN_KEY`/`APP_VISIBLE_CLASS` values so a rename can't drift
  silently. The same file measures the weight budget the identical way:
  gzip-compressing the real injected markup + CSS + script sliced out of
  the real build output, not a remembered number.

  **The numbers.** Phase A's bundle growth (CSS 10.42→14.05KB gzip
  2.89→3.59KB, main JS 244.68→252.26KB gzip 75.67→77.70KB) reverses:
  `landing.css`/`Landing.tsx` no longer sit in the app's own import graph
  at all, so nothing bundles them in, and the built numbers land back at
  10.42KB/2.89KB CSS and 244.95KB/75.74KB JS. The real first-paint cost —
  the injected markup + critical CSS + bootstrap script, gzip-measured —
  comes to **~5.7KB**, under §19's ~10KB budget. Frontend suite: 383 →
  385 (two Phase-A `App.test.tsx` cases and two `landing.test.ts` cases
  retired along with the client-rendered landing screen they tested; five
  new `prerender.test.ts` cases plus one new `landing.test.ts` drift-guard
  case added). `npm run build`, `npx vitest run`, and the AI-tell scanner
  all clean; backend `pytest` re-run as a sanity check (259, unaffected —
  no backend changes, per this step's own rule 1). Verified by hand too:
  a small jsdom smoke script exercising the actual built `dist/index.html`
  confirms a first-time visitor gets no visible class, a returning
  visitor (`localStorage` pre-set) gets `ms-app-visible` immediately, and
  clicking "Open the app" sets both the class and the storage flag.

### Phase C — the scan animation — **CODE DONE 2026-09-10, real-device tuning still needed**

- **14.7 The animated scan — done**, as a new `frontend/src/ScanAnimation.tsx`
  (used by `Landing.tsx`'s "How it works" section, section 3) plus the
  matching CSS in `landing.css`. One pinned inline SVG through the five
  states in §19 — photographed and rotated (state 1), straightened and
  detected (state 2), cells separated (state 3), digits resolved **with
  Q3 resolving to a flag rather than a number** (state 4), then the
  export (state 5, a mini spreadsheet row) — all five sharing one
  `viewBox` so they can crossfade via opacity in place, without a layout
  shift between them. Scroll-linked CSS only (`view-timeline`/
  `animation-timeline`, no JS); inline SVG only; no video, no PNG
  sequence. The grid now includes a **serial row** it didn't have in
  Phase A's static `ScanGraphic.tsx` — the reduced grid §19 specs for a
  phone (ID row, serial, three or four question columns) needed one, and
  since the same SVG serves both layouts (see below), it's there for wide
  too, which the spec explicitly allows ("the wider layout MAY show
  more" — not must, so one grid serving both was the simpler, still-
  compliant choice over building a second, richer wide-only variant).

  **Two layouts, phone first, via one shared structure**: `.lp-scan-
  scroller` is a column flexbox on phone — the pin sticks to `top: 0`
  bounded at `45vh` (never tall enough to push its own caption
  off-screen, since 45vh leaves the majority of a phone viewport for
  whatever caption is current) with the five captions (the SAME
  `.lp-step`/`.lp-list` markup the app already uses elsewhere, reused
  rather than duplicated — this is also why the pre-existing
  `Landing.test.tsx` assertions about 5 `.lp-step` items kept passing
  unmodified) scrolling beneath it. At `min-width: 64em` the scroller
  becomes a row, and `align-items: flex-start` is load-bearing: a plain
  flex row would stretch the short pin to match the much-taller captions
  column's height, which breaks `position: sticky` outright — a stickied
  element needs room to move within a container taller than itself while
  staying a fixed size.

  **The scroll-linking mechanism**: `.lp-scan-scroller` declares
  `view-timeline-name: --scan-progress; view-timeline-axis: block;`, and
  each `.scan-state-N` animates on that same named timeline with its own
  `@keyframes` distributing five (slightly overlapping, cross-fading)
  opacity windows across the 0–100% range. This is gated behind
  `@supports (animation-timeline: view()) and (prefers-reduced-motion:
  no-preference)` — outside that guard, EVERY state defaults to
  `opacity: 0` except `.scan-state-5`, which is `opacity: 1`. That default
  is not just the reduced-motion fallback (14.8): it's also what a
  browser without scroll-driven-animation support (older Firefox, Safari
  before v26) gets, and it's real, legible, static content — the settled
  export view — rather than a blank frame, satisfying progressive
  enhancement without a feature-detection script of any kind.

- **14.8 Reduced motion — done, shipped in the same commit as 14.7, not
  after it.** `prefers-reduced-motion: reduce` gets exactly the same
  fallback described above (state 5, statically) — it's the SAME
  `@supports`/`@media` guard doing double duty for "unsupported" and
  "motion declined", per the rule that these two must not be built as
  separate efforts.

  **What's honestly not yet true.** The crossfade window percentages
  (state 1 fading out around 22% of the timeline, state 2 taking over
  around 24–38%, and so on) are a reasoned first pass, not a tuned
  result — plan.md §19's own "Open risks" section and this step's Test
  section both say real-device scrolling is where this step is actually
  verified, not jsdom, which has neither a layout engine nor
  scroll-driven-animation support to check either against. Real-device
  work still needed, matching the step's own Done-when bar: confirming
  the crossfade timing feels right while actually scrolling (not just
  that the CSS is well-formed), checking the pinned graphic never
  overlaps its own caption on a real 320px phone, checking iOS Safari
  with the address bar both expanded and collapsed, and checking on a
  throttled 3G connection. None of that is simulatable from here.

  **What tests could verify, and do**: `ScanAnimation.test.tsx` (8 cases)
  checks all five states render with real content regardless of
  animation support, that the pin is `aria-hidden` (the captions already
  carry the same content as real text, so a screen reader isn't handed a
  single static description of a mid-crossfade illustration), that state
  4 genuinely renders a flag rather than a number for Q3, that state 5's
  export view never renders the flagged mark as a literal `0`, and that
  `landing.css` actually wires its `@supports`/reduced-motion fallback to
  these five exact class names — a drift guard, not a visual check.
  `prerender.test.ts` gained one more real-build-output assertion: all
  five `scan-state-N` classes are genuinely present in the built
  `dist/index.html`. Frontend suite: 385 → 394. The weight budget test
  still passes against the real build — the animation's SVG + CSS pushed
  the actual first-paint cost from ~5.7KB to **~7.9KB gzip**, still under
  §19's ~10KB bar but with less headroom than before, worth knowing if
  anything is added to this page later. `npm run build`, `npx vitest run`
  (three consecutive stable runs), the AI-tell scanner, and backend
  `pytest` (259, unaffected) all clean.

### 14.9 — Three real bugs, found by actually using it (2026-09-10)

Reported directly: "when we move past the landing page we cannot go there
anymore." Both causes were real, and neither was caught by any test in
14.1–14.8, because every test either exercised `Landing.tsx` as an
isolated component or read the real **build** output — nobody had opened
`npm run dev` and clicked through it, which is exactly the check CLAUDE.md
asks for on any frontend change and the one step 14.1–14.8 skipped.

**Bug 1 — the landing page didn't exist at all under `./dev.sh`.**
14.6's whole design is that `dist/index.html` gets the static shell
injected by `scripts/prerender-landing.mjs`, which runs after `vite
build`. `vite dev` never runs that script — it serves `index.html`
straight off disk with only its HMR client injected, so under the
*normal, everyday* dev workflow there was no `#landing-root`, no
bootstrap script, no critical CSS, anywhere. Combined with 14.6 having
removed `App.tsx`'s client-rendered `'landing'` screen entirely, the
practical result was: a first-time visitor to `npm run dev` never saw the
landing page, on any visit, ever. Fixed with a dev-mode Vite plugin
(`landingShellDevPlugin` in `vite.config.ts`, `apply: 'serve'` so it never
runs during `vite build`) that does the identical injection via
`server.ssrLoadModule('/src/prerenderEntry.tsx')` — Vite's own supported
way to run application source through its dev-time transform pipeline, no
second toolchain. The actual injection logic (`buildBootstrapScript`,
`injectLandingShell`) was factored out of `prerender-landing.mjs` into a
new shared `scripts/landing-shell.mjs`, imported by both the build script
and the dev plugin, so the two paths cannot silently drift apart on what
gets injected — the exact class of bug this fix itself was.
`landingShellDev.test.ts` (new, 2 cases) drives a real Vite dev server in
middleware mode (`createServer({ server: { middlewareMode: true } })`,
no port bound) and asserts the shell is genuinely served, the same way
`prerender.test.ts` asserts it against the real build.

**Bug 2 — even where the shell existed, the way back was undiscoverable.**
`Library.tsx`'s "Read the full story" link lived inside `<details
open={sections.length === 0}>` — the "How this works" disclosure, which
collapses itself the instant a first section exists. For any Library with
real data in it (which is to say: for any Library anyone would actually
be using), the only way back to the landing page was invisible unless you
already knew to expand a collapsed section labelled "How this works" and
scroll to its bottom. Moved to an always-visible "About" button in
`.app-header`, next to "+ New section" — never inside a collapsible
element again. `Library.tsx`/`Library.test.tsx`/`App.test.tsx` updated to
match (button text "About", not "Read the full story"); a new
`Library.test.tsx` case pins the actual fix directly: the button stays in
the document once a section exists, which is the state the bug lived in.

**Bug 3 — the scan animation's pin (and the topbar) never actually
stuck to anything, anywhere, ever.** Reported with screenshots: the
"How it works" graphic showed correctly beside step 1, then simply
disappeared for steps 2–5, scrolling away with everything else instead
of staying pinned. Root cause was one line, present since Phase A and
completely unrelated to the scan animation's own code:

```css
/* landing.css, before this fix */
.landing {
  overflow-x: hidden; /* belt-and-suspenders alongside body's own rule */
}
/* index.css, before this fix */
body {
  overflow-x: hidden;
}
```

Per the CSS Overflow spec, setting `overflow-x` to anything other than
`visible` while leaving `overflow-y` at its default forces the computed
value of `overflow-y` to `auto` — the two axes are coupled whenever
exactly one is non-`visible`. That silently turns both `body` and
`.landing` into scroll containers, and **any** scroll-container ancestor
between a `position: sticky` element and the real viewport confines that
element's stickiness to the ancestor's own box — which for an element
that never independently scrolls (both `body` and `.landing` just are
the page, at whatever height their content makes them) means the sticky
element doesn't stick at all; it just behaves like a normal
in-flow element. This broke `.lp-topbar` (the pinned "Open the app" bar,
decision 1's own core requirement, broken since Phase A) and
`.lp-scan-pin` identically, for the identical reason. It also breaks
`.data-table thead th`'s sticky header in the REAL APP's Results screen
(`index.css:684`, unrelated to step 14) — the same `body` rule, no
separate scroll container around the table — which had likely never
actually stuck either, on any real device, since nothing before this
could have caught it (jsdom renders no layout at all).

Fixed by replacing `overflow-x: hidden` with `overflow: clip visible` in
both places — `clip` clips the same horizontal overflow without ever
computing a non-`visible` value on the other axis, so the coupling never
triggers and neither element becomes a scroll container. `overflow: clip`
needs Chrome 90+/Firefox 97+/Safari 16+, all comfortably older than the
Safari 26 this step's own scroll-driven-animation CSS already assumes.

**Verified with a real browser, not by inspection** — jsdom cannot
render layout or evaluate `position: sticky` at all, so this fix could
not have been trusted on that basis alone. Used Playwright (Chromium,
already cached on this machine) to drive the actual `vite dev` server:
before the fix, `getComputedStyle(document.body).overflowY` read `auto`
and both `.lp-topbar` and `.lp-scan-pin`'s `getBoundingClientRect().top`
drifted in lockstep with `window.scrollY` (proof they were not sticking
at all — pinTop went 0.25 → -799.75 → -1999.75 across 2000px of
scrolling); after the fix, both stayed pinned at their `top` offset
(`0` and `80`) through the same 2000px scroll, `overflowY` read
`visible`, and the five `scan-state-N` opacities were confirmed
transitioning 1→2→3→4→5 in order while scrolling, settling at state 5.
Also confirmed no horizontal overflow reappeared at a real 320px
viewport (`document.documentElement.scrollWidth === clientWidth`,
before and after, scrolled through the whole page) — the property
`overflow-x: hidden` existed to guarantee in the first place.

Frontend suite: 394 → 397 (`landingShellDev.test.ts`'s 2 cases,
`Library.test.tsx`'s 1 new case — the sticky-positioning fix itself has
no jsdom-expressible regression test, consistent with this step's own
Test section naming real-device/real-browser verification as the only
way to check layout, not something added late). Verified by hand against
the real `vite dev` server (curled the served HTML directly, confirmed
`#landing-root`/`lp-bootstrap`/all five `scan-state-N` classes present)
and against `./local-stack.sh`'s real production build (rebuilt with the
correct `VITE_API_BASE` after the fix, confirmed the same markup served
there too). `npm run build`, `npx tsc -b`, three consecutive stable `npx
vitest run` passes, the AI-tell scanner all clean; backend untouched.

### 14.10 — the scan animation genuinely didn't work on a real phone (2026-09-10)

Reported with screenshots, on a real mobile viewport: the pinned graphic
showed correctly beside step 1's caption, then simply vanished for every
caption after it — huge stretches of blank space where the graphic
should have stayed pinned, matching what a broken `position: sticky`
looks like from the outside. 14.9 had already fixed the `overflow`
coupling that broke `position: sticky` site-wide, so this was a
*second*, narrower problem on top of that fix, specific to the
single-column phone layout.

**Verified with a real mobile viewport before touching anything** — the
instruction this time was explicit: use Playwright, actually render the
page, don't guess. `chromium.launch()` with Playwright's `devices['iPhone
13']` profile (390×844, already cached on this machine from 14.9's
investigation) reproduced a real, measurable problem: the pinned graphic
*did* technically stick and the five states *did* technically crossfade
in the right order (confirmed via `getComputedStyle` opacity reads across
a real scroll), so 14.9's own fix was working correctly — but the
`.lp-scan-captions .lp-step { min-height: 55vh }` rule, combined with a
single narrow column (no side-by-side layout to fill with a second
column's content the way wide screens have), produced multiple screens
of near-empty black space per caption. Technically-correct sticky
behavior surrounded by that much dead space reads as "broken," not as
"scroll to see more" — confirmed by an actual full-page screenshot
before the fix, matching the user's own report almost exactly.

**The fix, given explicit latitude to simplify** ("if needed don't keep
the points, just keep the animation"): rather than keep tuning the
single-column scroll-driven layout, phone stops being scroll-driven at
all. The graphic now plays as a **plain, self-contained, infinitely
looping CSS animation** — same five `@keyframes` the scroll-linked
version already used, just re-timed onto a normal 10s clock instead of
scroll position (`animation-timeline` left unset, so it runs on the
default document timeline — no `@supports` check needed, since a normal
looping `animation` has been universally supported for over a decade,
which sidesteps any real-mobile-Safari `animation-timeline: view()`
support question entirely rather than chasing it). The captions **stay
in the document** as an ordinary compact list directly below the
graphic, not one full-viewport-tall item per caption — dropping them
outright was offered but not taken, since the graphic is `aria-hidden`
and the captions are the only textual description of this section for a
screen reader; removing them would have traded a layout bug for an
accessibility regression. The wide (`min-width: 64em`) scroll-linked
version is completely unchanged — the `@supports (animation-timeline:
view())` block now overrides the base looping animation with the
scroll-driven one only there, and only when both motion is allowed and
the browser actually supports it; a wide-but-unsupported old browser
correctly falls back to the same looping animation phone gets, rather
than reintroducing giant caption spacers with nothing scroll-scrubbing
them.

**Verified again with the same real-browser method, both sizes**: on the
iPhone 13 profile, watched the five states cycle automatically over ~10
seconds with no scrolling at all, confirmed the caption list renders
immediately below the graphic with ordinary spacing (`.lp-scan-scroller`
height dropped from ~2197px to ~914px — a real 3rd-cut, not a marginal
one), and took a full-page screenshot confirming the whole page now
reads at one consistent density. On the 1400px desktop viewport,
re-confirmed the scroll-linked crossfade and sticky pin both still work
exactly as 14.9 left them, unaffected by this change.

Frontend suite unchanged at 397 — this fix, like 14.9's, is a real-
browser layout behavior with no meaningful jsdom expression, consistent
with this step's own Test section. `npm run build` (weight budget: added
CSS pushed first-paint gzip from ~7.9KB to ~8.9KB, still under the
~10KB bar but with less headroom left), `npx tsc -b`, three consecutive
stable `npx vitest run` passes, the AI-tell scanner all clean; backend
untouched. Verified against `./local-stack.sh`'s real production build
too, rebuilt with the correct `VITE_API_BASE` after the fix.

### Test

Per layer, as CLAUDE.md's testing rule requires:

- **`Landing.test.tsx`** — every section renders; the pinned CTA is
  present and calls through; the copy's factual claims are pinned as
  text assertions so a future edit can't quietly overclaim.
- **Entry logic** — a first-time visitor gets the landing; a visitor who
  has dismissed it goes straight to the library; the way back from the
  disclosure works. Storage cleared between cases.
- **The prerender guard** — a test that reads the **built**
  `dist/index.html` and asserts the landing content is genuinely in it.
  This is the one that catches the failure mode 14.5 introduces: a build
  that silently ships an empty shell while every component test still
  passes.
- **The weight budget** — asserted against the real build output, so
  §19's budget is enforced rather than remembered.
- **Reduced motion** — the query is honoured, checked directly rather
  than assumed.
- **Layout, at 320px** — no horizontal overflow, checked at the layout
  level rather than trusted to `body { overflow-x: hidden }`, which hides
  this symptom rather than preventing it.
- **By hand, on a real device — this is where this step is actually
  verified.** jsdom has no layout engine, so nothing above can catch a
  row running off the edge or a pinned graphic covering its own
  captions — the exact class of bug this repo has shipped three times.
  Required: the page on a real phone over a real throttled connection,
  the animation at 3G speed on a mid-range Android as well as an iPhone,
  the pinned bar and hero checked with iOS Safari's address bar both
  expanded and collapsed, and the returning-instructor path confirmed to
  show no flash of the landing page.

### Done when

A stranger sent the link can read what this is, how it works, and what it
refuses to do, **on a phone, held one-handed**, over 3G, with the first
paint arriving in roughly one round trip and no blank screen while a
bundle downloads. Nothing overflows horizontally at 320px, nothing jumps
when iOS Safari's address bar collapses, and the pinned graphic never
hides the caption explaining it. The
instructor who uses this every week sees it exactly once, reaches "Your
sections" in the same number of taps as before, and can still get back to
the explanation to show someone else. The animation degrades to a static
final state under reduced motion. The backend is untouched, no new
dependency was added, and the full first paint stays under the §19
budget.

---

## Step 15 — Crossed-out glyphs

**Goal.** A cell where the writer struck something out and wrote the
correction beside it ("~~6~~ 5") is read as the correction, offered to
the instructor for one tap, and never stored on its own. A crossed-out
ID digit becomes `?` rather than whatever digit the scribble resembles.

Raised 2026-09-23 from a real photo (Q1 "~~6~~ 5", Q2 "~~4~~ 3",
Q3 a struck 4 plus a second crossing-out touching the final 4). plan.md
§16's "Crossed-out glyphs" subsection carries the design. An extension of
the CNN track: the `remote`/`both` paths are unchanged, and nothing about
a cell with no crossing-out behaves differently.

### Before you start

Read `cnn/segment.py` and `cnn/decode.py` (the crossed-out glyph has to
be dropped between them), `app/recognizers/local.py`, `app/harvest.py`'s
N31 refusal (the crossed-out refusal is the same mechanism), and
`cnn/train.py`.

### Rules this step is bound by

1. **Flag, never guess, still governs.** What remains after dropping a
   struck glyph is a *suggestion* the instructor taps, never a stored or
   pre-filled value.
2. **Nothing already measured may get worse.** `cnn/accuracy.py` and
   `cnn/marks_accuracy.py` are re-run with the new model before it
   replaces the committed one, including the "confidently wrong" count.
3. **`testset/` is never trained on**, so it stays the honest held-out
   measure for both the old harnesses and the new one.
4. **A crossed-out crop is never harvested** under the instructor's
   correction.

### Substeps

- **15.1** `cnn/classes.py`: `CROSSED_OUT = 10`, appended after the digits
  so class index `d` still means digit `d` everywhere it is used.
  `model.py` reads `NUM_CLASSES` from it.
- **15.2** `cnn/pages.py`: photographed practice pages (loose digits in
  rows, no template) → labelled glyph crops, driven by
  `training_data/pages/pages.json` (gitignored with the photos).
- **15.3** `cnn/strikes.py`: synthetic strikes over EMNIST digits,
  rendered through inference's own `_to_canvas`, clean digits included.
- **15.4** `cnn/train.py`: the 11th class, practice pages oversampled,
  `--init-from` warm start widening a 10-class checkpoint,
  `--samples-per-epoch`, and per-epoch digit accuracy / false crossed-out
  rate / strikes caught reported separately.
- **15.5** `cnn/crossed_accuracy.py`: caught on held-out practice rows,
  false calls on every clean `testset/` glyph, `--sweep` over floors, and
  the full recognizer run over any `training_data/pages/template_*.jpg`.
- **15.6** `decode.py`'s `decide_digit` never returns `CROSSED_OUT` as a
  digit; `is_crossed_out(probs, floor)`; `CROSSED_OUT_FLOOR` in
  `thresholds.py`.
- **15.7** `local.py`: ID box → `?`; mark/total/serial → value `None`,
  struck glyphs dropped, the rest decoded as a suggestion. New
  `crossed_out_fields`/`suggestions` on `MarksResult`, `IdResult`,
  `ScanResult`.
- **15.8** `harvest.py` refuses `crossed_out_fields`; `HarvestFields`
  carries `crossedOutFields`.
- **15.9** `Review.tsx`: "Crossed out" plus **Use N** under a still-blank
  field; "A digit was crossed out" under the ID.

### Test

`tests/test_crossed_out.py` (routing, with the model replaced by a fake so
it is deterministic; 5 of its tests fail with detection disabled),
`tests/test_harvest.py`'s refusal cases, `Review.test.tsx`'s step 15
block. Model quality is `cnn/crossed_accuracy.py`, not a unit test.

### Done when

The template photo that raised this decodes to suggestions of 5 and 3
for Q1/Q2, with ID, serial and total unchanged; the held-out crossed-out
rows are mostly caught; clean `testset/` glyphs are almost never called
crossed out; and `cnn/accuracy.py`/`cnn/marks_accuracy.py` are no worse
on any line, confidently-wrong counts included.
...
## Step 17 — Partial scans, grid repair, and finding the decimal point

**Goal.** A scan should not become a dead end or a silent misread because
of one faint line or one small dot. Two strands, done 2026-09-25 from live
use: a column-count mismatch sent the instructor to retype every field,
and half marks were read poorly. plan.md §22 carries the design and the
safety rules. **Code-done 2026-09-25; the phone checks below remain.**

**Before you start.** `app/detection.py` (`_recover_bounds`,
`_repair_columns`, the crop loop), `app/main.py` (`scan`, `_is_partial`,
`harvest_endpoint`), `cnn/segment.py` (`segment_cell_detail`),
`app/recognizers/local.py` (`_resolve`), `frontend/src/Review.tsx`.

### 17.1 Partial scans (done)

A mismatch in one table no longer fails the scan. `detect()` writes no
crops for a miscounted table; `/api/scan` reads the tables that matched and
returns `table_mismatches`, the unread fields blank and flagged. Still a
full failure: every table miscounted, or a paper contradicting the
"Serial box on paper" setting (`_is_partial` — the ID is picked by
position). Gemini is never sent a miscounted marks table. Review shows a
banner naming the table in the terms a person counts ("found 6 digit
boxes, expected 7").

### 17.2 Grid repair (done)

`_repair_columns` fixes a table off by exactly one column: restore a line
the detector saw but rejected (above the absolute coverage floor), or
remove one that split a box — only when exactly one change gives evenly
spaced boxes AND both cells beside the change are the right width. Fixed
`real_class_11` (a 0.55-coverage divider dropped by the relative filter
under uneven light). The neighbour-cell condition was added after a test
showed a paper with one more ID box than the config would otherwise have
had its first digit merged into the label.

### 17.3 Save photo (done)

Failure and partial banners offer Save photo: a download of the exact
capture, named by reason and time. Real failed photos go in
`testset/private/` (gitignored).

### 17.4 Partial scans are harvested (done)

`/api/harvest` accepts the same partial result `/api/scan` does. No new
guard was needed: a miscounted table has no crops, and `harvest()` only
stores crops that exist, so a value typed into that table's fields has no
image to be attached to. A wrong Serial setting is still refused.

### 17.5 Weak decimal points (done)

The strong rule (small, lower half of the band) is unchanged. A WEAK point
is added on top: round, between two digit centres, below the top quarter
(people write it at mid-height); or rescued from under the noise floor
with >= 2% of a digit's ink; or a small round piece the pen-lift merge had
folded into a digit (a dot tucked under a 7's bar), taken back out with
its pixels blanked from the digit. `_resolve` reads a cell with and
without a weak point: one legal reading is the value, two become
`choices` (15 / 1.5 on a Total), shown as separate Use buttons and never
pre-filled. Stray ink between digits, and a glyph as wide as two touching
digits read differently when cut, only ever add choices. A tied field is
unmatched, so it is never harvested.

### 17.6 One folder for every question's crops (done)

Harvested question crops go in `marks_questions/` rather than one folder per
position (`marks_q1/`, ...). The position told a model nothing (not even the
question's max, which varies by quiz and isn't stored). `marks_total/` is
...
- `cnn/half_marks_accuracy.py` — the instructor's practice page
  (`training_data/pages/values.json`) plus every harvested mark cell.
- `tests/test_grid_repair.py` (mostly refusals), `test_cnn_segment.py`,
  `test_local_recognizer_resolve.py`, `test_main.py` partial-scan tests,
  `test_harvest_endpoint.py` partial-harvest tests,
  `frontend/e2e/scan-problems.spec.ts` (320px layout, a real download).
- A byte-identical before/after capture of detection over 99 runs.

### Done when

- [x] Every previously passing detection run byte-identical; only
  `real_class_11` changes, failed -> ok, read with no wrong digit.
- [x] Practice page: values stored wrong without a flag 2 -> 0.
- [x] Harvested whole marks: no new wrong reads; at most a tie where the
  cell genuinely looks like both.
- [x] Real testset marks unchanged (111/113, 0 wrong); ID and crossed-out
  unchanged.
- [ ] Save photo and a real partial scan checked on a phone.
- [ ] Half marks written on the printed grid scanned for real.

---

## Step 18 — Lighting: a blur check that measured darkness, a live hint, and the torch

**Goal.** Photos taken in poor light either scan, or say why they didn't —
and the instructor hears about a dark room or a shadow before tapping
Capture. Raised 2026-09-25 from a live session: 10 "blurry" failures in five
minutes (CloudWatch), gone once the room lights went on. plan.md §23 has the
design. **Code-done 2026-09-25; a real phone check (iPhone and Android) is
still needed, especially for the torch.**

### 18.1 The blur check measured brightness (done)

The old check (Laplacian variance < 50) scaled with contrast, so a sharp photo
in a dim room scored "blurry". Measured on the testset darkened to half
brightness: 18 of 28 rejected, all 28 readable. Replaced by
`detection._sharpness` — the same variance divided by the photo's own grey
variance — with `SHARPNESS_FLOOR = 0.115` between the 28 good photos (>= 0.134)
and `real_class_10`, the real blurry one (0.101). Darkened photos now score
higher, never lower. Every previously passing detection run is byte-identical.

**Revised the same day (18.4): no blur gate at all.** That 0.115 floor then
rejected three sharp photos from the instructor's own phone (0.093-0.107);
the grid read all three. Blurring every labelled photo gave zero wrong
unflagged values at any level, so every photo is now tried and `blurry`
only explains a failure (no grid found, sharpness < 0.03, light fine).
`real_class_10` — labelled blurry since step 0 — now reads correctly.

### 18.2 Say why (done)

A failed or partial scan carries `lighting`: `too_dark` (paper level, the
90th-percentile grey, under 70) or `uneven` (evenness, min/max paper level
over a 4x4 grid, under 0.6). Review turns it into one plain sentence. The
failure reasons themselves are unchanged.

### 18.3 Live hint and Light button (done)

`frontend/src/lighting.ts` measures the framing-guide region of the live video
(160x90, every 400 ms) with the same two measures, and shows "Too dark" or
"Shadow on the page" once two readings agree. It never blocks Capture. A
Light button appears only when the camera reports a torch, and the torch is
switched only by that tap — never by the app (the owner's decision).

### Test

`tests/test_lighting.py` (through /api/scan: dim-but-sharp now reads; very
dark fails as `too_dark`; heavy shadow as `uneven`; real blur still
`blurry`), `src/lighting.test.ts`, Review's lighting tests, and
`e2e/lighting.spec.ts` — a real browser whose fake camera plays generated
bright, dark and shadowed videos, plus a camera that reports a torch.

### Done when

- [x] No previously passing detection result changes; `real_class_10` still blurry.
- [x] A half-brightness photo that the old check rejected now scans.
- [x] The hint and the Light button behave in a real browser.
- [ ] Checked on a real iPhone and a real Android phone, torch included.
...
| 18 — Lighting: blur check, live hint, torch | **Code-done 2026-09-25; real iPhone/Android check needed.** **Revised same day: no up-front blur gate at all** — the 0.115 floor rejected three sharp photos from the instructor's phone (all readable); blurring every labelled photo gave zero wrong unflagged values at any level, so every photo is tried and `blurry` only explains a failure. real_class_10 now reads correctly. From a live session with 10 "blurry" failures in five minutes, cured by switching the room lights on: the old blur check (Laplacian variance < 50) measured brightness as much as focus (18/28 testset photos at half brightness rejected, 28/28 readable). Replaced by an exposure-independent sharpness score (floor 0.115: good photos >= 0.134, real_class_10 0.101); detection byte-identical on every passing run. Failed/partial scans now carry `lighting` (`too_dark`/`uneven`), explained on Review. Live hint over the viewfinder and a Light (torch) button, shown only when the camera has one, switched only by a tap. Backend 410 -> 418, frontend 468 -> 483, Playwright +4. |
| 17 — Partial scans, grid repair, decimal points | **Code-done 2026-09-25; phone checks remain.** A column-count mismatch in one table no longer fails the scan: the matched tables are read, and harvested, and the miscounted one is blank and flagged (`table_mismatches`); a wrong Serial setting still fails outright. `_repair_columns` fixes a table off by one from evidence only (fixed real_class_11). Save photo on the failure/partial banners. Half marks: weak decimal points (mid-height, under the noise floor, tucked inside a digit) read both ways; ties become `choices` (separate Use buttons). **Measured**: practice page (121 half marks) wrong 2 -> 0, correct 102 -> 114; harvested half 54 -> 58; harvested whole unchanged but one smudged Total "15" now offering 1.5/15; testset unchanged. Backend 310 -> 351, frontend 450 -> 456, Playwright 7. **Open**: a real partial scan and Save photo on a phone; half marks on the printed grid; a point written as a short dash is found only when it is in the lower half; fix C (neighbour-relative line strength) parked for more failed photos. |
...
| **Fixed — live-session pass (2026-09-09)** | **N31**, **N32**, **N33** — the three (of four) live-session findings that were fixable in code, done before step 13 rather than after: more sections means more serial reads hitting N32's bug and a longer collection window for N31 to poison. `decode_serial` (N32) now returns '?' per uncertain position, mirroring `read_id`, instead of blanking the whole field — 11/17 real serials survived the old rule though 14/17 were correct at raw argmax. `_decode_value_cell` (N31/N33) now distinguishes a blank cell from ink that matched no legal value; `harvest()` refuses to label a crop in that set regardless of what the instructor typed to get past Confirm, and `Review.tsx` explains the blank instead of leaving it silent. **N34 was explicitly NOT fixed** — asked to choose a fix direction (out-of-band/defer/automated-capture), the user chose to defer. See issues.md's "Fixed 2026-09-09" writeup for the full account. |
| **Fixed — N35 (2026-09-10)** | A single-digit mark written with a leading zero ("03", "05") could never decode on the default `cnn` path — `decode_value` only ever scored a legal value's canonical (never zero-padded) digit rendering, so a 2-glyph "03" reading had no length-matching candidate to compare against for the value 3, structurally rather than by confidence. Fixed by having `decode.py`'s new `_digit_candidates` try each legal value both as-is and with one leading zero prepended (decimal position shifted along with it), scored like any other candidate — the padded leading glyph still has to classify as a genuine "0" to win. Found by checking a direct user question, not an audit pass; half marks (0.5, 1.5, 2.5, …) were checked in the same pass and were **already working**. See issues.md's "Fixed 2026-09-10 — N35" writeup for the full account. |
| Still open | **5.** N34 (Med, deferred by explicit choice, 2026-09-09) plus four Low deploy/infra (N11, N15, N22, N23). The desk audits closed everything on the `cnn` path; the live session found four real findings, three of which are now fixed, and N35 (found separately) is fixed too. |
| Test suites | **259 backend + 339 frontend**, from 148 + 79. Both passed before the audits, which is the point worth internalising rather than a footnote. |

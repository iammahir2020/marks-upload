...
### Phase A — the page, rendered client-side

Shippable on its own: the story exists and can be read, reviewed and
iterated before any build plumbing changes.

- **14.1 Landing tokens.** A scoped dark palette extending the app's own
  warm dark values with a deeper canvas step beneath `--background`,
  built as a four-step surface ladder with hairline borders and **no**
  drop shadows. Plus the **fluid** display ramp (`clamp()`, ~32px on a
  phone up to 64px on a wide screen — never a desktop size scaled down)
  and the section rhythm starting at **48px** and growing to 64 and 96 via
  `min-width` queries. All scoped under the landing root.
- **14.2 `Landing.tsx`.** The seven sections from §19, in order, plus the
  pinned top bar carrying "Open the app" at every scroll position — one
  compact row that does not grow on scroll, since vertical space is the
  scarce resource on a phone. The phone hero is **headline + one line +
  CTA**, with the animation peeking from the bottom edge rather than
  fitting; the two-column arrangement arrives at a `min-width`. Purely
  presentational — no hooks touching browser APIs during render, since
  14.5 will run this exact component through `renderToStaticMarkup` in
  Node.
- **14.3 The copy.** Written, not lorem — the hook is that the marking is
  already done and what remains is transcription. Every factual claim
  must be one the code actually keeps (§19's section 4 and 5 both make
  claims that are invariants elsewhere in this repo; nothing may be
  claimed that isn't).
- **14.4 Entry and exit.** `Screen` gains `'landing'`; it is the initial
  screen for someone who has never dismissed it, and never shown again
  once they have. **A way back exists** — a link from the library's
  existing "How this works" disclosure — so the explanation is reachable
  when someone wants to show a colleague.

### Phase B — static-first delivery

The performance half. Nothing a reader sees changes; what changes is when
they see it.

- **14.5 The prerender step.** A small Node script run after `vite build`
  that renders `Landing.tsx` with `renderToStaticMarkup` and injects the
  markup plus its critical CSS into `dist/index.html`. Wired into
  `npm run build` so there is no second command to forget.
- **14.6 Zero runtime JS for the landing.** The markup moves *outside*
  `#root`, so React never owns it, hydrates it, or re-renders it.
  Visibility is a class on `<html>`, set before first paint by a small
  inline script reading a **synchronous `localStorage` flag** — not the
  async IndexedDB `meta` store, which would paint the page and then yank
  it away. The CTA is handled by that same inline script, so it works
  before the main bundle has arrived. After this substep `Landing.tsx` is
  a build-time input only and ships no runtime chunk.

### Phase C — the scan animation

- **14.7 The animated scan.** One pinned inline SVG through the five
  states in §19 — photographed and rotated, straightened and detected,
  cells separated, digits resolved **with one resolving to a flag rather
  than a number**, then the export. Scroll-linked CSS only; inline SVG
  only. A video or PNG sequence is explicitly out of scope for weight.
  **Two layouts, phone first**: the graphic pins to the top at a bounded
  height with captions scrolling beneath it — never tall enough to push
  its own caption off-screen — and only moves beside the captions at a
  `min-width`. The grid drawn on a phone is **reduced** (ID row, serial,
  three or four question columns) so it stays legible at 320px; the wide
  layout may show more.
- **14.8 Reduced motion.** `prefers-reduced-motion: reduce` renders the
  final state statically. Ships with 14.7, not after it.

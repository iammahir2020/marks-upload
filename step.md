...
| 18 — Lighting: blur check, live hint, torch | **Code-done 2026-09-25; real iPhone/Android check needed.** From a live session with 10 "blurry" failures in five minutes, cured by switching the room lights on: the old blur check (Laplacian variance < 50) measured brightness as much as focus (18/28 testset photos at half brightness rejected, 28/28 readable). Replaced by an exposure-independent sharpness score (floor 0.115: good photos >= 0.134, real_class_10 0.101); detection byte-identical on every passing run. Failed/partial scans now carry `lighting` (`too_dark`/`uneven`), explained on Review. Live hint over the viewfinder and a Light (torch) button, shown only when the camera has one, switched only by a tap. Backend 410 -> 418, frontend 468 -> 483, Playwright +4. |
